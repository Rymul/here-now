...
const EventIndexMap = ({latlng, events, pins}) => {
    ...
    window.pinsDropped= pinsDropped;
    ...
    useEffect(()=> {
        ...
        if (latlng.lat) {bounds.extend(new window.google.maps.LatLng(latlng.lat, latlng.lng));}
        ...
        if (Object.values(pins.current).length > 0 && !bounds.isEmpty() && !!map) {
    ...
    useEffect(() => {
        ...
        let bounds = new window.google.maps.LatLngBounds();
        if (latlng.lat && !!map) {
            if (Object.values(pins.current).length === 0) {
            ...
            if (Object.values(pins.current).length === 0 && !pinsDropped) {
        ...
        if (marker.current === null && !!map && latlng.lat) {
